...
const recordHls = (config, streams) => {
  ...
  }).on('add', (path) => {console.log('<<<>>Chokidar watchman<<>>'); onFile(path, 'add', mediaRoot, streams)})
    .on('change', (path) => {console.log('<<>>Chokidar watchman<<>>'); onFile(path, 'change', mediaRoot, streams)});
};
